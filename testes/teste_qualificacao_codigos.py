...
def _base_receitas() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "bloco": ["receitas"] * 7,
            "aba_origem": ["Receitas"] * 7,
            "codigo_ibge": ["3500000"] * 7,
            "municipio": ["Municipio A"] * 7,
            "ano": [2024] * 7,
            "estagio": ["Realizadas"] * 7,
            "codigo_conta": ["1.1.1.2.50", pd.NA, pd.NA, pd.NA, pd.NA, pd.NA, pd.NA],
            "descricao_conta": [
                "IPTU",
                "Total das Receitas",
            ...
            "rotulo_conta_original": [
                ...
                "Receitas brutas | RECEITAS (EXCETO INTRA-ORCAMENTARIAS) (I)",
                "Realizadas | 1.7.1.8.03.0.0\nTransferencia de Recursos do SUS ¿.1",
            ],
            "origem_metadados": ["dicionario"] * 7,
            "valor": [100.0, 300.0, 50.0, 200.0, 80.0, 500.0, 60.0],
            "valor_original": [100.0, 300.0, 50.0, 200.0, 80.0, 500.0, 60.0],
        }
    )
...
def teste_qualificacao_separa_pendencias_e_funcao(tmp_path: Path) -> None:
    ...
    despesas = receitas.assign(
        bloco="despesas",
        aba_origem="Despesas",
        codigo_conta=["3.3.90.30.00", pd.NA, pd.NA, "3.3.91.39.00", pd.NA, pd.NA, pd.NA],
        descricao_conta=[
            "Material de Consumo",
            "Total das Despesas",
            "Descricao sem codigo",
        ...
        rotulo_conta_original=[
            ...
            "Descricao sem codigo 2",
            "Empenhadas | DESPESAS (EXCETO INTRAORCAMENTARIAS)",
            "Descricao sem codigo 3",
        ],
    )
    ...
    funcao = receitas.assign(
        ...
        codigo_conta=["15.451", pd.NA, pd.NA, pd.NA, pd.NA, pd.NA, pd.NA],
        descricao_conta=[
        ...
        rotulo_conta_original=[
            ...
            "Pagas | DESPESAS (INTRA-ORCAMENTARIAS)",
            "Descricao sem codigo 2",
        ],
    )
    ...
    assert quadro_receitas.loc[5, "tipo_registro"] == "subtotal_estrutural"
    assert not bool(quadro_receitas.loc[5, "pendencia_revisao"])
    assert quadro_receitas.loc[5, "natureza_operacao"] == "orcamentaria"
    assert quadro_receitas.loc[6, "codigo_conta"] == "1.7.1.8.03.0.0"
    ...
    assert quadro_despesas.loc[5, "tipo_registro"] == "subtotal_estrutural"
    assert quadro_despesas.loc[5, "natureza_operacao"] == "orcamentaria"
    ...
    assert quadro_funcao.loc[5, "tipo_registro"] == "subtotal_estrutural"
    assert quadro_funcao.loc[5, "natureza_operacao"] == "intraorcamentaria"
    assert not bool(quadro_funcao.loc[5, "pendencia_revisao"])
